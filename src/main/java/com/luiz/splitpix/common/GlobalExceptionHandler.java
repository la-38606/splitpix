 ...
 * must fail loudly in tests rather than silently degrade.
 */
...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ErrorResponse> handleMissingHeader(MissingRequestHeaderException e) {
		if ("Idempotency-Key".equalsIgnoreCase(e.getHeaderName())) {
			return respond(HttpStatus.BAD_REQUEST, "IDEMPOTENCY_KEY_REQUIRED");
		}
	...
	public ResponseEntity<ErrorResponse> handleIntegrityViolation(DataIntegrityViolationException e) {
		...
		// Log only the first line of the cause: it names the constraint but,
		// unlike the Detail line, never contains column values (Pix keys).
		log.warn("constraint violation: {}", firstLine(NestedExceptionUtils.getMostSpecificCause(e).getMessage()));
		return respond(HttpStatus.CONFLICT, "CONSTRAINT_VIOLATION");
	}

	@ExceptionHandler({ DataAccessException.class, TransactionException.class })
	public ResponseEntity<ErrorResponse> handleDatabase(Exception e) {
	...
	private ResponseEntity<ErrorResponse> respond(HttpStatus status, String code) {
		String message = messageSource.getMessage("error." + code, null, PT_BR);
