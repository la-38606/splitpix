...
`DebtSimplifier.simplify` repeatedly matches the largest debtor with the
largest creditor (two priority queues, deterministic tie-break by participant
id) and transfers the smaller magnitude. This guarantees at most n-1 transfers
in O(n log n). Suggestions are computed per request and never stored.
...
A 300-seed randomized property test (`DebtSimplifierTest`) checks that applying
every suggestion zeroes all balances, amounts are positive, and nobody pays
themselves.
